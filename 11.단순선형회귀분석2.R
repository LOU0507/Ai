...
summary(model.4)
# 독립변수중에서 기여도가 가장 높은 변수는 ? HS.Grad
# 우의 수준은? 1.057e-05
# 모델의 설명력? 0.3597
# 회귀식에서 독립변수를 HS.Grad만 구하기
model.5 <- lm(Income~HS.Grad,data.frame(state.x77))
model.5
summary(model.5)

#4-2
model.6 <- lm(Income~Illiteracy + Poplation, data=data.frame(state.x77))
model.6
summary(model.6)
...
Illiteracy <- 2.1
Population <- 3615
pred = -168.63466*Illiteracy + 0.03555*Population + 4833.16224
pred
...
# --------------------------------
# 로지스틱 회귀분석
# y 값이 범주형인 것 분석하기  (범주형의 자료를 정수형으로 변환함. 월 -1 화 -2, 동 -1 서 -2...)
# --------------------------------
# 예측하기 - 품종
head(iris)
class(iris)
...
pred <- predict(model.7,unknown) # predict ( 데이터셋, 예측하려는 x 값들)
pred #0.9174506
pred <- round(pred,0)
pred
...
y = -0.11191*Sepal.Lenght+-0.04008*Sepal.Width+0.22865*Petal.Length+0.60925*Petal.Width+1.18650
y #0.917439
y <- round(y,0)
y
...
ds$Type <-  as.integer
#(1)Glass#(1) set.seed(100)을 실행한 후 전체 데이터(관측값)에서 임의로 60%를 추출하여 train에 저장하고 나머지 40%는 test에 저장하시오.
set.seed(100)
...
model.glass <- glm(Type~.,data=train)
summary(model.glass)
...
head(pred)
head(test)
#(4) pred.val#(4) 예측한 값과 실제 잔디의 종류를 비교하여 모델의 정확도(accuracy)가 어느 정도인지 확인하시오.
