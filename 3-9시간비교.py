...
#속도 향상을 위한 여러 기법을 동원한다. 세 가지 기법을 제시한다.
...
이런 간접 비용으로 인해 my_cvtGray1은 느리다. 반면 배열 연산을 사용하는 my_cvtGray2 
함수는 배열의 모든 요소는 같은 데이터 형이라는 사실에 따라, 연산을 시작하기전에 데이
...
라면 다음 요소를 접근핛 때는 현재 요소 주소에 4를 더하면 된다.

③ 캐시를 사용하여 접근 시간을 줄인다. 주기억 장치보다 빠른 캐시에 여러 요소를 한꺼번에
