...
class Orim(QMainWindow):
    def __init__(self) :
        ...
        painButton=QPushButton('페인팅',self)
        ...
        painButton.clicked.connect(self.painFunction)
    ...
    def fileOpenFunction(self):
        ...
        if self.img is None : sys.exit('파일을 찾을 수가 없습니다.')
    ...
    def painting(self,event,x,y,flags,param):
        if event==cv.EVENT_LBUTTONDOWN:
            ...
            cv.circle(self.mask,(x,y),self.BrushSiz,cv.GC_PR_FGD,-1)
            
        elif event==cv.EVENT_RBUTTONDOWN:
            cv.circle(self.img_show,(x,y),self.BrushSiz,self,cv.GC_FGD,-1)
        ...
        elif vent==cv.EVENT_MOUSEMOVE and flags==cv.EVENT_FLAG_RBUTTON:
