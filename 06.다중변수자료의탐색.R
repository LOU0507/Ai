...
iris.2<- iris[,3:4]
iris.2
point <- as.numeric(iris$Species) #as.numeric('품종의종류')숫자로 비꿔라
point
...
#월별 지각생으 ㅣ수를 선그래프로 표현
month=1:12
late=c(5,8,7,9,4,6,12,13,8,6,6,4)
plot(month,late,main = "월별 지각생수",type='l',lty=1,lwd=4,col="#ff0000")

#시나리오 
#미국 보스턴 지역 주택 가격에 영향을 미치는 항목은 무엇인가?
#외부에 있는 데이터셋의 패키지 다운로드
install.packages("mlbench")
library(mlbench) # R studio에다가 해당 라이브러리 사용(활성시키겠다)하겠다는 의미
data('BostonHousing')
head(BostonHousing)
dim(BostonHousing)
class(BostonHousing)
...
#4.차트로 그리기 연속형 자료니깐 히스톨그램, boxplot
par(mfrow=c(2,3))
for(i in 1:5){
